...
class DiscordBridgeBot(commands.Bot):
    def __init__(self):
        super().__init__(
            ...
            help_command=None,
            activity=discord.Game(name="RNGHere Bridge Bot")
        )
    ...
    async def send_user_message(
        ...
        #rewritten to handle images, very WIP, needs my chattriggers module to work

    async def send_minecraft_user_message(self, username, message: discord.Message, *, officer: bool = False):
        content = message.content        

        # Base64 encode the URL to bypass the restriction of using '.' in hypixel chat (yay hypixel)
        def base64_encode(url: str) -> str:
            return base64.urlsafe_b64encode(url.encode("utf-8")).decode("utf-8").rstrip("=")  # Strip '=' padding

        # If there are no attachments and the content is empty, return
        if content.strip() == "" and not message.attachments:
        ...
        if content.strip():
            ...
            # Replace user mentions found in the content
            ...
            # Replace role mentions found in the content
            ...
            # Replace channel mentions found in the content
            ...
            def link_replacer(match):
                link = match.group(0)
                return f"[回]{base64_encode(link)}[回]"

            # Use the link regex to find and replace links in the content
            content = link_regex.sub(link_replacer, content)

            # Handle message references (replies)
            if message.reference:
                reply = message.reference.cached_message or None
                try:
                    if not reply:
                        reply = await message.channel.fetch_message(message.reference.message_id)
                except discord.HTTPException:
                    pass
                if reply and reply.author != self.user:
                    reply_to = "@" + reply.author.name
                    username += f" replied to {reply_to}"

            # Add the officer or guild command prefix
            if officer:
                content = f"/gc {username}: {content}"
            else:
                content = f"/gc {username}: {content}"

            # Clean up the content before sending it to Minecraft
        ...
        #trims format at the end of discord urls to try and make them shorter
        def discordURLTrim(url):
            return re.sub(r'&format.*', '', url)
        ...
        if message.attachments:
            for attachment in message.attachments:
                ...
                # Encode the URL
                encoded_url = base64_encode(attachment_url)

                # Try and trim URL
                trimmedURL = discordURLTrim(encoded_url)

                # Send the encoded URL as a separate message in Minecraft
                chat_command = f"/gc {username}: [回]{trimmedURL}[回]"
                # Prevent getting the bot kicked for sending too long of a message
                if len(chat_command) >= 255:
                    chat_command = f"/gc {username}: sent an image, but it was too long to send the link. Thank you hypixel. :("
                # Que message for async sending
                await self.mineflayer_bot.chat(chat_command)
